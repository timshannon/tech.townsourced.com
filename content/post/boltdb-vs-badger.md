...
wanted to build something with Badger in the future, and when a issue was opened to 
...
users to switch between the two via a configuration flag, or build an entirely separate library.  I end up following
the Go proverb:
...
I found Badger to use significantly more memory than BoltDB.  So much so that  I had to rewrite my tests from using a 
separate database for each test to using a single shared database for all tests.  My 8GB VM that I ran tests on would
fail to complete due to running out of memory every single time, even after changing all of Badger's options to their
...
and Badger. There are many scenarios where this trade off is well worth the cost, however, I was personally surprised by 
it, as I hadn't seen it mentioned in any of the documentation or blog posts.
